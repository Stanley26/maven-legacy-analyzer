...
`StructureIndex` projette dans l'index de chaque module les modèles archivés accessibles depuis son POM par héritage/import de BOM, les relations d'agrégation et la disponibilité de l'arbre Maven. Il ne parcourt ni le cache ni les sources originales. Les relations de collecte fournissent les coordonnées interpolées lorsqu'elles sont disponibles ; les déclarations symboliques restent inconnues autrement. Les POM des dépendances sans lien avec cette hiérarchie ne sont pas inclus dans cette projection.

`structures.js` normalise les coordonnées des modules locaux en rôles de packaging, conserve les chaînes ordonnées de parents externes et les arbres d'agrégation (avec multiplicités), puis regroupe les hiérarchies identiques. Les variantes conservent BOM/profils/ordre d'import et usages de dépendances par rôle de module, version/type/classifier/scope, chemin et origine documentée. Les modèles d'un autre module du même dépôt servent de repli uniquement si leurs descriptions de parents/imports sont concordantes. Les cycles, parents non résolus, agrégations manquantes ou ambiguës empêchent le classement comme hiérarchie complète.

Les familles sont construites dans l'ordre décroissant des effectifs des hiérarchies, avec départage déterministe. Chaque nouveau groupe est rapproché du meilleur groupe de référence existant : parent commun, puis BOM commun, puis forte similarité de dépendances directes (au moins trois, composition identique et Jaccard pondéré ≥ 0,75). Le poids est `1 + ln((N + 1) / (fréquence + 1))`. Un type de module terminal commun est requis. Les groupes ne sont pas fusionnés transitivement. Les résultats décrivent des ressemblances techniques, pas des familles métier, la conformité ou une compatibilité de migration. Les observations incomplètes restent visibles et leurs absences ne produisent aucun retrait de dépendance/BOM.
