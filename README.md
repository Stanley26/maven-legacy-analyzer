...
Les nouveaux scans regroupent les journaux Maven dans `evidence/<projet>/<module>/logs/`. Les évaluations de coordonnées de parents ou de BOM ont leur propre sous-dossier `logs/coordinates/`, qui contient les journaux `coordinate-*.log` (y compris les tentatives `coordinate-*-cache.log`) et leurs résultats `coordinate-*.txt`. Leur présence n'indique pas une erreur. Les autres diagnostics, notamment la recherche des parents et du cache, restent dans `logs/`. Les expressions d'un même contexte sont regroupées dans des appels de taille limitée ; les valeurs des modèles externes ne sont pas partagées entre différents contextes Maven. `context.pomGraph` indique le nombre de propriétés et de lots évalués. Les anciens rapports conservent leurs chemins et restent consultables sans déplacement de leurs preuves.
