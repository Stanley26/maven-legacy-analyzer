...
public class Main implements Runnable {
    ...
    public static class Scan implements Callable<Integer> {
        ...
        @Option(names = {"-o", "--output"}, required = true, description = "Dossier de sortie neuf ou vide") Path output;
        ...
        public Integer call() throws Exception {
            ...
            for (var module : scan.modules()) {
                System.out.printf("[%d/%d] %s%n", ++index, scan.modules().size(), module.id);
                if (!inventoryOnly) {
                    try {
                        var options = configuration.forRepository(Path.of(module.repository));
                        String key = HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(module.id.getBytes(StandardCharsets.UTF_8))).substring(0, 20);
                        new Collector().collect(module, options, output.resolve("evidence").resolve(key), offline);
                    } catch (Exception ex) {
